...
<form class="d-flex" form action= "form-2A_4_1.php" method="POST">
...
<?php
     if($_SESSION['designation']== "Director"){
                ?>
               <li class="active"><a href="\mpms-nitttr-kolkata\form-2A_4.php"><span class=""></span> View All Staff Performance</a></li>
               <?php
      } 
?>
        ...
        <h4> 
            <ull>
                <a href="director.php">  Director  </a> >
                 <lii><a href="form-2A_4.php"> View All Staff Performance </a></lii>
            </ull>
            <center>
        <h1 class="my-4">View All Staff Performance</h1>
    </center><hr> 
    <br>
        </h4>
<?php
    ...
    <h4> 
        <ull>
          <a href="controlling_office.php">  Controlling Office  </a> >
              <lii><a href="form-2A_4.php"> View Staff Performance </a></lii>
        </ull>
        <center>
        <h1 class="my-4">View Staff Performance</h1>
    </center><hr> 
    <br>
    </h4>
<?php
...
?>
            ...
            <form action="/mpms-nitttr-kolkata/form-2A_4_1.php" method="post" id="form">
...
if ($designation == "Staff"){
    ...
    ?>  
                        ...
                        <div class="col-12 mt-5">
                                ...
                                <input type="submit" class="btn btn-primary btn-lg"  action="/mpms-nitttr-kolkata/form-2A_4_1.php" formvalue="Submit" value="Enter"/>                
                                </div>
                              </div>
                            </div>
                        </div>
        ...
        <form action="/mpms-nitttr-kolkata/form-2A_4_1.php" method="post">
            <div class="card mt-5">
                <div class="card-body">
                        ...
                        <div class="row mt-5 gy-5">
                            ...
                            <div class="col-9">
                                ...
                                $staff_name =$_POST['staff_name'];
                                    echo "<b style='font-size: 18px;'>".$staff_name."</b>";
                                ?>
                                <!-- </span> -->
                            </div><br><br><br><br>
                    ...
                    <tr>
                        ...
                        <th scope="col">No of Pending Jobs</th>
                        <th scope="col">No of Completed Jobs</th>
                        <th scope="col">No of Cancel Jobs</th>
                    ...
                    <tbody>
                           ...
                           #echo $staff_name;
                            $sql = "SELECT `staff_name`,`job_type`,`job_status` FROM `job_details` Where `job_details`.`staff_name` ='$staff_name' && job_type ='Short Type'";
                            $result = mysqli_query($mysqli, $sql);
                            $row = mysqli_fetch_assoc($result);
                            $sql1 = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`='Pending' && `staff_name`='$staff_name' && `job_type`='Short type'";
                            $result1 = mysqli_query($mysqli, $sql1);
                            $row1 = mysqli_fetch_assoc($result1);
                            $sql2 = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`='Completed' && `staff_name`='$staff_name' && `job_type`='Short type'";
                            $result2 = mysqli_query($mysqli, $sql2);
                                 $row2 = mysqli_fetch_assoc($result2);
                            $sql3 = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`= 'Cancel' &&`staff_name`='$staff_name' && `job_type`='Short type'";
                            ...
                            <td>".$sno."</td>
                            <td>".$row["job_type"]."</td> 
                            <td>".$row1["job_status"]."</td>
                            <td>".$row2["job_status"]."</td>
                            <td>".$row3["job_status"]."</td>
                            <td>".$row2["score"]."</td> ";
                           

                            $sql = "SELECT `staff_name`,`job_type`,`job_status` FROM `job_details` Where `job_details`.`staff_name` ='$staff_name' && job_type ='Medium Type'";
                            $result = mysqli_query($mysqli, $sql);
                            $row = mysqli_fetch_assoc($result);
                            $sql1 = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`='Pending' && `staff_name`='$staff_name' && `job_type`='Medium Type'";
                            $result1 = mysqli_query($mysqli, $sql1);
                            $row1 = mysqli_fetch_assoc($result1);
                            $sql2 = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`='Completed' && `staff_name`='$staff_name' && `job_type`='Medium Type'";
                            $result2 = mysqli_query($mysqli, $sql2);
                            $row2 = mysqli_fetch_assoc($result2);
                            $sql3 = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`='Cancel' && `staff_name`='$staff_name' && `job_type`='Medium type'";
                            $result3 = mysqli_query($mysqli, $sql3);
                            $row3 = mysqli_fetch_assoc($result3);
                            ...
                            $sql = "SELECT `staff_name`,`job_type`,`job_status` FROM `job_details` Where `job_details`.`staff_name` ='$staff_name' && job_type ='Long Type'";
                            $result = mysqli_query($mysqli, $sql);
                            $sql1 = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`='Pending' && `staff_name`='$staff_name' && `job_type`='Long Type'";
                            $result1 = mysqli_query($mysqli, $sql1);
                            $row1 = mysqli_fetch_assoc($result1);
                            $sql2 = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`='Completed' && `staff_name`='$staff_name' && `job_type`='Long Type' && `remarks` !='Cancel'";
                            $result2 = mysqli_query($mysqli, $sql2);
                            $row2 = mysqli_fetch_assoc($result2);
                            $sql3 = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`='Cancel' && `staff_name`='$staff_name' && `job_type`='Long Type'";
                            $result3 = mysqli_query($mysqli, $sql3);
                            $row3 = mysqli_fetch_assoc($result3);
             ...
             # $sql = "SELECT count(*) AS `job_status` FROM `job_details` WHERE `job_status`='Completed' && `staff_name`='' && `job_type`='short type'";
